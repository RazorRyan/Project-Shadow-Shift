...
export function getMovementTuning(state: any, player: any): MovementTuning {
  ...
  if (player.moveSpeed != null) tuning.moveSpeed = player.moveSpeed;
  if (player.jumpForce != null) tuning.jumpForce = player.jumpForce;
  if (player.dashSpeed != null) tuning.dashSpeed = player.dashSpeed;
  if (player.dashDuration != null) tuning.dashDuration = player.dashDuration;
  if (player.dashCooldownDuration != null) tuning.dashCooldownDuration = player.dashCooldownDuration;
  if (player.wallSlideSpeed != null) tuning.wallSlideSpeed = player.wallSlideSpeed;
  if (player.jumpCutMultiplier != null) tuning.jumpCutMultiplier = player.jumpCutMultiplier;
  return tuning;
}
...
export function resolveHorizontalVelocity(player: any, movement: MovementTuning, moveAxis: number, dt: number): number {
  ...
  if (player.attackRecover > 0) targetSpeed += player.facing * movement.attackRecoverBoost;
